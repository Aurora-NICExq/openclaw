...
describe("right-click Reply", () => {
  ...
  it.each(["reply", "Escape"] as const)(
    "restores composer focus after %s from a menu retained across a render",
    (action) => {
      const flushFrames = stubAnimationFrames();
      const onSetReply = vi.fn();
      const props = createChatProps({ onSetReply });
      const container = document.createElement("div");
      document.body.append(container);
      try {
        render(renderChat(props), container);
        const { bubble } = appendChatBubble(container, { text: "retained reply source" });
        dispatchContextMenu(bubble);
        flushFrames();
        const menu = expectDefined(
          document.querySelector<HTMLElement>(".chat-reply-context-menu"),
          "retained menu",
        );
        ...
        render(renderChat({ ...props, draft: "draft after refresh" }), container);
  ...
  it("does not focus a detached section through a retained reply menu", () => {
    const container = renderChatView({ onSetReply: vi.fn() });
    const otherInput = document.createElement("input");
    document.body.append(container, otherInput);
    try {
      const { bubble } = appendChatBubble(container, { text: "detached reply source" });
      dispatchContextMenu(bubble);
      const reply = expectDefined(
        document.querySelector<HTMLButtonElement>(".chat-reply-context-menu button"),
        "retained reply action",
      );
      const textarea = expectDefined(container.querySelector("textarea"), "composer");

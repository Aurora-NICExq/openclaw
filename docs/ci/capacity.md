...
Ordinary and real-Gateway Control UI jobs retain captured failure diagnostics for seven days, with separate artifact paths for each job and run attempt. These JSON reports and screenshots are diagnostic artifacts; they are not sanitized public proof.

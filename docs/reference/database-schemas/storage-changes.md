...
Automatic entry maintenance captures its policy and protected session identities
at writer admission, then plans on the existing reclamation worker. Protection
includes runtime providers, active work, and active lifecycle mutations; the
parent rechecks these owners and the write generation before planning commits.
...
Incognito databases, prepared native deletion hooks, commit-authorization joins,
archive publication bookkeeping, and repository/worktree cleanup retain their
existing parent-side owners.

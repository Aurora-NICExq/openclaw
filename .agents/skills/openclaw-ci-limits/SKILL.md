...
- `ci-gate` always uses `ubuntu-24.04` for its Bash-only result aggregation,
  ...
  outside eligible hybrid first attempts. Security hooks use pinned installed packages
  ...
  hybrid first attempt and one per normal Blacksmith run. Both jobs already occur
...
- Eligible Control UI E2E rows request the 16-vCPU class with unchanged live
  ...
  stays on 8. The twelve-row 4:38 result in historical run 33695337496 used the

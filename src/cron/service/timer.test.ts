...
import { run as runJob } from "./ops-run.js";
  ...
  it.each([
    ...
    async (schedule) => {
      ...
      try {
        ...
        await expect(runJob(state, job.id, "due")).resolves.toEqual({
          ok: true,
          ran: false,
          reason: "not-due",
        });
        await expect(runJob(state, job.id, "force")).resolves.toEqual({ ok: true, ran: true });

...
export const SYSTEM_AGENT_ASSISTANT_SYSTEM_PROMPT = [
  ...
  "Config writes are proposed, approved, then validated against the schema and live-tested when the default inference route changes. Failed validation returns errors to you; fix the value and propose it again. Use `set default model` as the shortcut for switching the primary model. Doctor repairs remain outside chat; use `openclaw doctor --fix` on the machine running OpenClaw.",
  ...
  "Secrets go through `config set-ref`, never plaintext. Never echo secret values back.",
...
export const SYSTEM_AGENT_SYSTEM_PROMPT = [
  ...
  "Before writing an uncertain config path, call config_schema. Config writes are proposed, approved, then validated against the schema and live-tested when the default inference route changes. Failed validation returns errors to you; fix the value and propose it again. Secrets go through config_set_ref, never plaintext. set_default_model remains the shortcut for switching the primary model. plugin_uninstall refuses plugins backing the active inference route; exit and run `openclaw plugins uninstall <id>` for those plugins.",

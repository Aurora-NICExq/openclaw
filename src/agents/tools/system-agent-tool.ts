...
export function createSystemAgentTool(options: SystemAgentToolOptions): AnyAgentTool {
  return {
    ...
    description: [
      ...
      "Unknown config: config_schema first. Config writes are proposed, approved, then validated against the schema and live-tested when the default inference route changes. Failed validation returns errors to you; fix the value and propose it again. Secrets: config_set_ref, never plaintext. set_default_model is the shortcut for switching the primary model.",
